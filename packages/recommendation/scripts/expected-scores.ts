  ...
  for (const s of scored.slice(0, 5)) {
    console.log(
      `${s.recipe.id}: score=${s.score.toFixed(6)} passed=${s.passed} ` +
        ...
        `time=${s.breakdown.timeMatch.value.toFixed(4)} pref=${s.breakdown.preferenceMatch.value.toFixed(4)} variety=${s.breakdown.varietyScore.value.toFixed(4)} novelty=${s.breakdown.noveltyScore.value.toFixed(4)}`,
    );
  }
  console.log('');
}
